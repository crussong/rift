...
function save() {
    ...
    try { localStorage.setItem('rift-5e-v3', JSON.stringify(S)); } catch (e) {}
}
...
function load() {
    ...
    if (typeof CharacterStorage !== 'undefined') {
        try {
            ...
            if (stored && stored.data) {
                ...
                migrateState();
                applyPortrait();
                console.log('[5e-v3] Loaded from CharacterStorage:', _charId);
                return;
            }
        } catch (e) {}
    }
    ...
    try {
        const d = localStorage.getItem('rift-5e-v3');
        if (d) {
            S = JSON.parse(d);
            migrateState();
            applyPortrait();
            console.log('[5e-v3] Loaded from localStorage');
            return;
        }
    } catch (e) {}
...
function initRiftLink() {
    if (!window.RIFT || !RIFT.link || !RIFT.state) {
        console.log('[5e-v3] RiftLink not available, offline mode');
        return;
    }
    if (!_roomCode) {
        console.log('[5e-v3] No room code, offline mode');
        return;
    }
    if (!_charId) {
        console.warn('[5e-v3] No character ID for RiftLink');
        return;
    }
    ...
    console.log('[5e-v3] RiftLink connected:', _charId, 'in', _roomCode);
    ...
    RIFT.state.on('riftlink:char:updated', (ev) => {
        ...
        try {
            ...
            applyPortrait();
            render();
            console.log('[5e-v3] Applied remote update:', ev.fields?.join(', '));
        } finally {
...
function initSheet() {
    ...
    render();
    initRiftLink();
    console.log('[5e-v3] Sheet initialized', _charId ? '(char: ' + _charId + ')' : '(new)');
}
