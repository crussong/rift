...
const RiftInventory = (() => {
    ...
    function loadStarterKit() {
        ...
        // Binding sizes: Waffen 2x4, Brust 2x3, Hose 2x3, Kopf 2x2,
        // Gürtel 2x1, Tränke 1x2, Ringe 1x1, Amulette 1x1, Nebenhand 2x3
        const demo = [
            // Waffen: gridW:2, gridH:4
        ...
        // Force-update: replace existing demo items (fix sizes from old data)
        let added = 0, updated = 0;
        for (const d of demo) {
            d.col = null; d.row = null;
            const idx = ch.inventory.items.findIndex(i => i.instanceId === d.instanceId);
            if (idx !== -1) {
                // Preserve quantity if stackable, force-update everything else
                const oldQty = ch.inventory.items[idx].quantity;
                ch.inventory.items[idx] = d;
                if (d.stackable && oldQty > d.quantity) ch.inventory.items[idx].quantity = oldQty;
                updated++;
            } else {
                ch.inventory.items.push(d);
                added++;
            }
        }

        // Re-pack all placements
        ch.inventory.items.forEach(i => { i.col = null; i.row = null; });
        _save(ch);
        _notify(`Demo: ${added} neu, ${updated} aktualisiert`);
    }
