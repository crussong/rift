...
const DICE = (function() {
    ...
    // RIFT PRO: Gemstone Texture - high-quality faceted gem surfaces
    function generateGemTexture(ctx, width, height, baseColor, facetColor, highlightColor, gemType) {
        // Deep base fill
        ctx.fillStyle = baseColor;
        ctx.fillRect(0, 0, width, height);
        
        // Inner depth glow - radial gradient from center
        var depthGlow = ctx.createRadialGradient(
            width * 0.45, height * 0.4, 0,
            width * 0.5, height * 0.5, width * 0.6
        );
        depthGlow.addColorStop(0, facetColor);
        depthGlow.addColorStop(0.5, baseColor);
        depthGlow.addColorStop(1, 'rgba(0,0,0,0.3)');
        ctx.globalAlpha = 0.6;
        ctx.fillStyle = depthGlow;
        ctx.fillRect(0, 0, width, height);
        ctx.globalAlpha = 1.0;
        
        // === Per-gem unique characteristics ===
        switch(gemType) {
            case 'ruby':
                // Deep internal fire - warm secondary glow
                var fireGlow = ctx.createRadialGradient(width*0.6, height*0.35, 0, width*0.5, height*0.5, width*0.4);
                fireGlow.addColorStop(0, 'rgba(255,60,20,0.25)');
            ...
            case 'sapphire':
                // Star asterism - 6-pointed star highlight
                ctx.save();
                ctx.globalAlpha = 0.2;
                ctx.strokeStyle = highlightColor || '#ffffff';
                ctx.lineWidth = 1.5;
                ...
                ctx.shadowBlur = 8;
                var cx = width * 0.48, cy = height * 0.42;
                for (var ray = 0; ray < 6; ray++) {
                    var angle = (ray / 6) * Math.PI * 2 - Math.PI / 2;
                    ctx.beginPath();
                    ctx.moveTo(cx - Math.cos(angle) * width * 0.25, cy - Math.sin(angle) * height * 0.25);
                    ctx.lineTo(cx + Math.cos(angle) * width * 0.25, cy + Math.sin(angle) * height * 0.25);
                    ctx.stroke();
                }
                ctx.restore();
                break;
            ...
            case 'emerald':
                // Jardin inclusions - fine internal cracks/gardens
                ctx.globalAlpha = 0.12;
                ctx.strokeStyle = '#88cc88';
                ctx.lineWidth = 0.8;
                for (var inc = 0; inc < 6; inc++) {
                    ctx.beginPath();
                    var ix = width * 0.2 + Math.random() * width * 0.6;
                    var iy = height * 0.2 + Math.random() * height * 0.6;
                    ctx.moveTo(ix, iy);
                    for (var seg = 0; seg < 4; seg++) {
                        ix += (Math.random() - 0.5) * 25;
                        iy += (Math.random() - 0.5) * 25;
                        ctx.lineTo(ix, iy);
                    }
                    ctx.stroke();
                }
                // Feathery wisp inclusions
                ctx.globalAlpha = 0.06;
                ...
                for (var w = 0; w < 3; w++) {
                    ...
                    ctx.moveTo(wx, wy);
                    ctx.bezierCurveTo(wx + 30, wy - 20, wx + 60, wy + 20, wx + 80, wy);
                    ctx.stroke();
                }
                ctx.globalAlpha = 1.0;
                break;
            ...
            case 'amethyst':
                // Color zoning - lighter to darker bands
                ctx.globalAlpha = 0.15;
                for (var z = 0; z < 5; z++) {
                    var zoneY = z * height / 5;
                    ctx.fillStyle = z % 2 === 0 ? 'rgba(200,150,255,0.15)' : 'rgba(80,20,120,0.15)';
                    ctx.fillRect(0, zoneY, width, height / 5);
                }
                // Subtle chevron growth pattern
                ctx.strokeStyle = 'rgba(200,170,255,0.08)';
                ctx.lineWidth = 2;
                for (var ch = 0; ch < 4; ch++) {
                    var cy2 = height * 0.2 + ch * height * 0.2;
                    ctx.beginPath();
                    ctx.moveTo(0, cy2);
                    ctx.lineTo(width / 2, cy2 - 15);
            ...
            case 'diamond':
                // Prismatic rainbow fire - scattered spectral flashes
                var prismColors = ['rgba(255,0,0,0.06)', 'rgba(0,200,0,0.06)', 'rgba(0,80,255,0.06)',
                                   'rgba(255,150,0,0.06)', 'rgba(200,0,255,0.06)', 'rgba(0,200,200,0.06)'];
                for (var p = 0; p < 10; p++) {
                    var px = Math.random() * width;
                    var py = Math.random() * height;
                    var pSize = 8 + Math.random() * 20;
                    ctx.fillStyle = prismColors[Math.floor(Math.random() * prismColors.length)];
                    ctx.beginPath();
                    // Triangle flash shapes
                    ctx.moveTo(px, py - pSize / 2);
                    ctx.lineTo(px + pSize / 2, py + pSize / 3);
                    ctx.lineTo(px - pSize / 2, py + pSize / 3);
                    ctx.closePath();
                    ctx.fill();
                }
                // Extra brilliance - bright center
                var brilliance = ctx.createRadialGradient(width*0.45, height*0.38, 0, width*0.5, height*0.5, width*0.3);
                brilliance.addColorStop(0, 'rgba(255,255,255,0.15)');
                brilliance.addColorStop(1, 'rgba(255,255,255,0)');
                ctx.fillStyle = brilliance;
                ctx.fillRect(0, 0, width, height);
                break;
                
            case 'topaz':
                // Warm inner glow - honeyed light
                var topazGlow = ctx.createRadialGradient(width*0.4, height*0.35, 0, width*0.5, height*0.5, width*0.5);
                topazGlow.addColorStop(0, 'rgba(255,220,120,0.2)');
                topazGlow.addColorStop(0.5, 'rgba(255,180,60,0.08)');
                topazGlow.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = topazGlow;
                ctx.fillRect(0, 0, width, height);
                // Soft basal cleavage lines
                ctx.globalAlpha = 0.06;
                ctx.strokeStyle = '#ffdd88';
                ctx.lineWidth = 1;
                for (var t = 0; t < 3; t++) {
                    ctx.beginPath();
                    ctx.moveTo(0, height * 0.3 + t * 15);
                    ctx.lineTo(width, height * 0.35 + t * 15);
                    ctx.stroke();
                }
                ctx.globalAlpha = 1.0;
                break;
            ...
            case 'opal':
                // Play of color - iridescent patches
                var opalHues = [
                    'rgba(255,80,80,0.12)', 'rgba(80,255,80,0.12)', 'rgba(80,80,255,0.12)',
                    'rgba(255,200,50,0.12)', 'rgba(50,200,255,0.12)', 'rgba(255,80,200,0.12)'
                ];
                for (var oi = 0; oi < 12; oi++) {
                    var ox = Math.random() * width;
                    var oy = Math.random() * height;
                    var oRad = 8 + Math.random() * 25;
                    var oGrd = ctx.createRadialGradient(ox, oy, 0, ox, oy, oRad);
                    var oColor = opalHues[Math.floor(Math.random() * opalHues.length)];
                    oGrd.addColorStop(0, oColor);
            ...
            case 'onyx':
                // Banded layers - alternating dark/lighter bands
                ctx.globalAlpha = 0.12;
                for (var band = 0; band < 8; band++) {
                    var bY = band * height / 8 + (Math.random() - 0.5) * 4;
                    var bH = height / 16 + Math.random() * 5;
                    ctx.fillStyle = band % 2 === 0 ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.12)';
                    ctx.fillRect(0, bY, width, bH);
                }
                // Subtle silky luster
                var silkGlow = ctx.createLinearGradient(0, 0, width, height * 0.6);
                silkGlow.addColorStop(0, 'rgba(255,255,255,0)');
                silkGlow.addColorStop(0.4, 'rgba(255,255,255,0.06)');
                silkGlow.addColorStop(0.6, 'rgba(255,255,255,0.06)');
                silkGlow.addColorStop(1, 'rgba(255,255,255,0)');
                ctx.globalAlpha = 1.0;
                ctx.fillStyle = silkGlow;
                ctx.fillRect(0, 0, width, height);
                break;
                
            case 'tourmaline':
                // Bicolor - pink/green watermelon split
                ctx.globalAlpha = 0.2;
                // Pink zone (top-left)
                var pinkZone = ctx.createRadialGradient(width*0.3, height*0.3, 0, width*0.4, height*0.4, width*0.5);
                pinkZone.addColorStop(0, 'rgba(255,100,150,0.3)');
                pinkZone.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = pinkZone;
                ctx.fillRect(0, 0, width, height);
                // Green zone (bottom-right)
                var greenZone = ctx.createRadialGradient(width*0.7, height*0.7, 0, width*0.6, height*0.6, width*0.5);
                greenZone.addColorStop(0, 'rgba(80,220,100,0.3)');
                greenZone.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = greenZone;
                ctx.fillRect(0, 0, width, height);
                // Transition zone in middle
            ...
            case 'alexandrite':
                // Color change effect - two dominant hue zones blending
                ctx.globalAlpha = 0.25;
                var alexGreen = ctx.createRadialGradient(width*0.3, height*0.5, 0, width*0.4, height*0.5, width*0.5);
                alexGreen.addColorStop(0, 'rgba(0,180,120,0.35)');
                alexGreen.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = alexGreen;
                ctx.fillRect(0, 0, width, height);
                var alexRed = ctx.createRadialGradient(width*0.7, height*0.5, 0, width*0.6, height*0.5, width*0.5);
                alexRed.addColorStop(0, 'rgba(200,50,100,0.35)');
                alexRed.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = alexRed;
                ctx.fillRect(0, 0, width, height);
                ctx.globalAlpha = 1.0;
                break;
        }
        
        // === Universal facets - overlapping polygonal shapes ===
        var facetCount = gemType === 'diamond' ? 12 : 8;
        for (var f = 0; f < facetCount; f++) {
            var fcx = width * 0.15 + Math.random() * width * 0.7;
            var fcy = height * 0.15 + Math.random() * height * 0.7;
            var sides = 3 + Math.floor(Math.random() * 4); // 3-6 sides
            var fRad = width * 0.06 + Math.random() * width * 0.14;
            var angleOff = Math.random() * Math.PI * 2;
            
            ctx.beginPath();
            for (var s = 0; s < sides; s++) {
                var fAngle = angleOff + (s / sides) * Math.PI * 2;
                var fx = fcx + Math.cos(fAngle) * fRad;
                var fy = fcy + Math.sin(fAngle) * fRad;
                if (s === 0) ctx.moveTo(fx, fy);
                else ctx.lineTo(fx, fy);
            }
            ctx.closePath();
            
            // Alternating light/dark facets for 3D depth
            if (f % 3 === 0) {
        ...
        ctx.save();
        ctx.shadowColor = highlightColor || '#ffffff';
        ctx.shadowBlur = 10;
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(width * 0.38, height * 0.32, width * 0.015, 0, Math.PI * 2);
        ctx.fill();
        
        // Secondary highlight
        ctx.globalAlpha = 0.2;
        ctx.beginPath();
        ctx.arc(width * 0.62, height * 0.55, width * 0.01, 0, Math.PI * 2);
        ctx.fill();
        
        // Tiny scatter highlights
        ctx.shadowBlur = 4;
        for (var h = 0; h < 4; h++) {
        ...
        var edgeVignette = ctx.createRadialGradient(
            width * 0.48, height * 0.45, width * 0.15,
    ...
    // RIFT PRO: Glass Texture - translucent glass surfaces
    function generateGlassTexture(ctx, width, height, baseColor, highlightColor, tintColor, glassType) {
        // Base fill
        ctx.fillStyle = baseColor;
        ctx.fillRect(0, 0, width, height);
        
        // Central light gathering - gives illusion of translucency
        var centerLight = ctx.createRadialGradient(
            width * 0.4, height * 0.35, 0,
            width * 0.5, height * 0.5, width * 0.55
        );
        centerLight.addColorStop(0, 'rgba(255,255,255,0.12)');
        centerLight.addColorStop(0.5, 'rgba(255,255,255,0.04)');
        centerLight.addColorStop(1, 'rgba(0,0,0,0.08)');
        ctx.fillStyle = centerLight;
        ctx.fillRect(0, 0, width, height);
        
        // === Per-glass unique characteristics ===
        switch(glassType) {
            case 'clear':
                // Clean with subtle refraction distortion
                for (var ci = 0; ci < 3; ci++) {
                    var cx2 = Math.random() * width;
                    var cy2 = Math.random() * height;
                    var cRad = 15 + Math.random() * 25;
                    var cGrd = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, cRad);
                    cGrd.addColorStop(0, 'rgba(255,255,255,0.06)');
                    cGrd.addColorStop(0.5, 'rgba(200,220,255,0.03)');
            ...
            case 'frosted':
                // Matte diffuse surface - fine noise
                var fd = ctx.getImageData(0, 0, width, height);
                var fp = fd.data;
                for (var i = 0; i < fp.length; i += 4) {
                    var fn = (Math.random() - 0.5) * 22;
                    fp[i] = Math.min(255, Math.max(0, fp[i] + fn));
                    fp[i+1] = Math.min(255, Math.max(0, fp[i+1] + fn));
                    fp[i+2] = Math.min(255, Math.max(0, fp[i+2] + fn));
                }
                ctx.putImageData(fd, 0, 0);
                // Soft overall bloom
                var frostBloom = ctx.createRadialGradient(width*0.5, height*0.5, 0, width*0.5, height*0.5, width*0.45);
                frostBloom.addColorStop(0, 'rgba(255,255,255,0.08)');
                frostBloom.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = frostBloom;
                ctx.fillRect(0, 0, width, height);
                break;
                
            case 'stained':
                // Stained glass panels with lead lines
                ctx.save();
                // Create irregular panels
                var panelPts = [];
                for (var pp = 0; pp < 8; pp++) {
                    panelPts.push({ x: Math.random() * width, y: Math.random() * height });
                }
                // Color each voronoi-like region
                var stainColors = [
                    'rgba(220,40,40,0.15)', 'rgba(40,80,220,0.15)', 'rgba(40,180,60,0.15)',
                    'rgba(220,180,30,0.15)', 'rgba(180,40,180,0.15)', 'rgba(40,180,200,0.15)',
                    'rgba(220,120,40,0.15)', 'rgba(200,200,200,0.08)'
                ];
                for (var sp = 0; sp < panelPts.length; sp++) {
                    var sGrd = ctx.createRadialGradient(
                        panelPts[sp].x, panelPts[sp].y, 0,
                        panelPts[sp].x, panelPts[sp].y, width * 0.25
                    );
                    sGrd.addColorStop(0, stainColors[sp % stainColors.length]);
                    sGrd.addColorStop(1, 'rgba(0,0,0,0)');
                    ctx.fillStyle = sGrd;
                    ctx.fillRect(0, 0, width, height);
                }
                // Lead lines (dark borders between panels)
            ...
            case 'murano':
                // Swirling color spirals trapped inside
                ctx.save();
                ctx.globalCompositeOperation = 'screen';
                var muranoColors = [tintColor || '#ff6644', '#ffaa22', '#44aaff'];
                for (var mc = 0; mc < 5; mc++) {
                    ctx.globalAlpha = 0.12;
                    ctx.strokeStyle = muranoColors[mc % muranoColors.length];
                    ctx.lineWidth = 3 + Math.random() * 4;
                    ctx.beginPath();
                    var mx = width * 0.3 + Math.random() * width * 0.4;
                    var my = height * 0.3 + Math.random() * height * 0.4;
                    ctx.moveTo(mx, my);
                    for (var ms = 0; ms < 8; ms++) {
                        var mAngle = ms * 0.8;
                        var mRad = ms * 5 + Math.random() * 8;
                        ctx.quadraticCurveTo(
                            mx + Math.cos(mAngle) * mRad * 1.5, my + Math.sin(mAngle) * mRad * 1.5,
                            mx + Math.cos(mAngle + 0.4) * mRad, my + Math.sin(mAngle + 0.4) * mRad
                        );
                    }
                    ctx.stroke();
                }
            ...
            case 'seaglass':
                // Frosted, tumbled, soft edges - very matte organic
                var sd = ctx.getImageData(0, 0, width, height);
                var sp = sd.data;
                for (var i = 0; i < sp.length; i += 4) {
                    var sn = (Math.random() - 0.5) * 30;
                    sp[i] = Math.min(255, Math.max(0, sp[i] + sn));
                    sp[i+1] = Math.min(255, Math.max(0, sp[i+1] + sn));
                    sp[i+2] = Math.min(255, Math.max(0, sp[i+2] + sn));
                }
                ctx.putImageData(sd, 0, 0);
                // Soft surface pitting
                ctx.globalAlpha = 0.06;
                for (var pit = 0; pit < 30; pit++) {
                ...
                ctx.globalAlpha = 1.0;
                // Heavy edge softening
                var seaEdge = ctx.createRadialGradient(width*0.5, height*0.5, width*0.2, width*0.5, height*0.5, width*0.6);
                seaEdge.addColorStop(0, 'rgba(255,255,255,0.06)');
                seaEdge.addColorStop(1, 'rgba(0,0,0,0.08)');
                ctx.fillStyle = seaEdge;
                ctx.fillRect(0, 0, width, height);
                break;
                
            case 'crackle':
                // Internal crack network - spider web pattern
                ctx.save();
                ctx.globalAlpha = 0.18;
                ctx.strokeStyle = highlightColor || '#ffffff';
                ctx.lineWidth = 0.8;
                // Generate connected crack network
                var crackNodes = [];
                for (var cn = 0; cn < 12; cn++) {
                    crackNodes.push({ x: width*0.1 + Math.random()*width*0.8, y: height*0.1 + Math.random()*height*0.8 });
                }
                for (var c1 = 0; c1 < crackNodes.length; c1++) {
                    for (var c2 = c1 + 1; c2 < crackNodes.length; c2++) {
                        var cdist = Math.sqrt(Math.pow(crackNodes[c1].x - crackNodes[c2].x, 2) + Math.pow(crackNodes[c1].y - crackNodes[c2].y, 2));
                        if (cdist < width * 0.35) {
                            ctx.beginPath();
                            ctx.moveTo(crackNodes[c1].x, crackNodes[c1].y);
                            // Slightly jagged line
                            var cmx = (crackNodes[c1].x + crackNodes[c2].x) / 2 + (Math.random()-0.5) * 8;
                            var cmy = (crackNodes[c1].y + crackNodes[c2].y) / 2 + (Math.random()-0.5) * 8;
                            ctx.quadraticCurveTo(cmx, cmy, crackNodes[c2].x, crackNodes[c2].y);
                            ctx.stroke();
                        }
                    }
                }
                ...
                ctx.shadowColor = highlightColor || '#ffffff';
                ctx.shadowBlur = 6;
                for (var c3 = 0; c3 < crackNodes.length - 1; c3++) {
                    var cdist2 = Math.sqrt(Math.pow(crackNodes[c3].x - crackNodes[c3+1].x, 2) + Math.pow(crackNodes[c3].y - crackNodes[c3+1].y, 2));
                    if (cdist2 < width * 0.35) {
                        ctx.beginPath();
                        ctx.moveTo(crackNodes[c3].x, crackNodes[c3].y);
                        ctx.lineTo(crackNodes[c3+1].x, crackNodes[c3+1].y);
                        ctx.stroke();
                    }
                }
                ctx.restore();
            ...
            case 'crystal':
                // Faceted crystal cuts - geometric reflections
                ctx.save();
                for (var cf = 0; cf < 10; cf++) {
                    var cfx = Math.random() * width;
                    var cfy = Math.random() * height;
                    var cfSides = 4 + Math.floor(Math.random() * 3);
                    var cfRad = width * 0.05 + Math.random() * width * 0.12;
                    var cfAngle = Math.random() * Math.PI;
                    
                    ctx.beginPath();
                    for (var cfs = 0; cfs < cfSides; cfs++) {
                        var cfa = cfAngle + (cfs / cfSides) * Math.PI * 2;
                        var cfpx = cfx + Math.cos(cfa) * cfRad;
                        var cfpy = cfy + Math.sin(cfa) * cfRad;
                        if (cfs === 0) ctx.moveTo(cfpx, cfpy);
                        else ctx.lineTo(cfpx, cfpy);
                    }
                    ctx.closePath();
                    
                    // Prismatic fill
                    var cfGrd = ctx.createLinearGradient(cfx - cfRad, cfy, cfx + cfRad, cfy);
                    cfGrd.addColorStop(0, 'rgba(255,255,255,0.1)');
                    cfGrd.addColorStop(0.5, 'rgba(200,220,255,0.06)');
                    cfGrd.addColorStop(1, 'rgba(0,0,0,0.04)');
                    ctx.fillStyle = cfGrd;
                    ctx.fill();
            ...
            case 'smoke':
                // Dark swirling wisps inside
                ctx.save();
                ctx.globalCompositeOperation = 'multiply';
                for (var sw = 0; sw < 5; sw++) {
                    ctx.globalAlpha = 0.08;
                    var swx = Math.random() * width;
                    var swy = Math.random() * height;
                    var swRad = width * 0.15 + Math.random() * width * 0.2;
                    var swGrd = ctx.createRadialGradient(swx, swy, 0, swx, swy, swRad);
                    swGrd.addColorStop(0, 'rgba(0,0,0,0.2)');
                    swGrd.addColorStop(0.5, 'rgba(40,40,50,0.1)');
                ...
                for (var st = 0; st < 3; st++) {
                    ctx.beginPath();
                    var stx = Math.random() * width;
                    var sty = Math.random() * height;
                    ctx.moveTo(stx, sty);
                    for (var ss = 0; ss < 4; ss++) {
                        ctx.bezierCurveTo(
                            stx + (Math.random()-0.5)*60, sty + (Math.random()-0.5)*40,
                            stx + (Math.random()-0.5)*60, sty + (Math.random()-0.5)*40,
            ...
            case 'amber':
                // Warm trapped bubbles and organic inclusions
                ctx.save();
                // Warm depth glow
                var amberGlow = ctx.createRadialGradient(width*0.45, height*0.4, 0, width*0.5, height*0.5, width*0.5);
                amberGlow.addColorStop(0, 'rgba(255,200,80,0.15)');
                amberGlow.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = amberGlow;
                ctx.fillRect(0, 0, width, height);
                // Trapped air bubbles
                for (var ab = 0; ab < 12; ab++) {
                    var abx = width * 0.15 + Math.random() * width * 0.7;
                    var aby = height * 0.15 + Math.random() * height * 0.7;
                    var abr = 1 + Math.random() * 4;
                    ctx.globalAlpha = 0.12;
                    ...
                    ctx.fillStyle = '#ffffff';
                    ctx.beginPath();
                    ctx.arc(abx - abr * 0.3, aby - abr * 0.3, abr * 0.3, 0, Math.PI * 2);
                    ctx.fill();
                }
                ctx.restore();
                break;
            ...
            case 'obsidian':
                // Volcanic glass - near black with razor-sharp reflections
                // Sharp linear reflections
                ctx.save();
                ctx.globalCompositeOperation = 'screen';
                for (var ol = 0; ol < 4; ol++) {
                    var olGrd = ctx.createLinearGradient(
                        Math.random()*width, Math.random()*height,
                        Math.random()*width, Math.random()*height
                    );
                    olGrd.addColorStop(0, 'rgba(255,255,255,0)');
                    olGrd.addColorStop(0.45, 'rgba(255,255,255,0)');
                    olGrd.addColorStop(0.5, 'rgba(255,255,255,0.12)');
                    olGrd.addColorStop(0.55, 'rgba(255,255,255,0)');
                    olGrd.addColorStop(1, 'rgba(255,255,255,0)');
                    ctx.fillStyle = olGrd;
                    ctx.fillRect(0, 0, width, height);
                }
                // Conchoidal fracture marks
                ctx.globalAlpha = 0.05;
                ctx.strokeStyle = '#555566';
                ctx.lineWidth = 1;
                for (var oc = 0; oc < 3; oc++) {
                    ctx.beginPath();
                    ctx.arc(Math.random()*width, Math.random()*height, 10+Math.random()*20, 0, Math.PI * 0.8);
                    ctx.stroke();
                }
                ctx.restore();
                break;
        ...
        // === Universal specular highlight - diagonal light streak ===
        if (glassType !== 'frosted' && glassType !== 'seaglass') {
            ctx.save();
            ctx.globalCompositeOperation = 'screen';
            ...
            streak.addColorStop(0.45, 'rgba(255,255,255,0.12)');
        ...
        // === Bubble inclusions (for types that have them) ===
        if (glassType !== 'stained' && glassType !== 'crackle' && glassType !== 'obsidian') {
            ctx.globalAlpha = 0.08;
            for (var b = 0; b < 5; b++) {
                var bx = width * 0.15 + Math.random() * width * 0.7;
                var by = height * 0.15 + Math.random() * height * 0.7;
                var br = 0.8 + Math.random() * 2.5;
        ...
        // === Edge darkening ===
        var glassEdge = ctx.createRadialGradient(
            width * 0.48, height * 0.45, width * 0.18,
            width * 0.5, height * 0.5, width * 0.62
        );
        glassEdge.addColorStop(0, 'rgba(0,0,0,0)');
        glassEdge.addColorStop(1, 'rgba(0,0,0,0.12)');
        ctx.globalAlpha = 1.0;
            ...
            if (vars.dice_texture && vars.dice_texture.type) {
                ...
                switch(tex.type) {
                    ...
                    case 'runic':
                        ...
                        break;
                    case 'gem':
                        generateGemTexture(context, ts, ts, tex.baseColor || '#330011', tex.facetColor || '#660022', tex.highlightColor || '#ffffff', tex.gemType || 'ruby');
                        break;
                    case 'glass':
                        generateGlassTexture(context, ts, ts, tex.baseColor || '#e8eef4', tex.highlightColor || '#ffffff', tex.tintColor || '#aaccee', tex.glassType || 'clear');
                        break;
                    default:
